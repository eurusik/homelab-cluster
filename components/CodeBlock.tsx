...
export default function CodeBlock({ code, language = 'bash' }: CodeBlockProps) {
  ...
  return (
    <div className="relative bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg overflow-hidden max-w-full">
      {/* Copy button */}
      <button
        onClick={handleCopy}
        className="absolute top-3 right-3 p-2 hover:bg-[#2a2a2a] rounded transition-colors group"
        aria-label="Copy code"
      >
        {copied ? (
          <svg className="w-5 h-5 text-[#4aff4a]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Code content */}
      <div className="p-4 pr-14 font-mono text-sm overflow-x-auto">
